...
exports = module.exports = function(opts){
	...
	gt.kill = function(childProcess){
		...
		if(!isWindows) {
			killProcess = spawn("kill", ["-9", pid]);
		} else {
			killProcess = spawn("taskkill", ["/pid", pid, "/f",
								"/t"]);
		}
		if(opts.log) {
			var popts = { stdout: true, stderr: true };
			return promisify(killProcess, popts).then(function(result){
				if(result.stdout) {
